...
const AccordianContent = ({ data }) => {
    ...
    return <>
        {
            data.map((i) => (
                <div key={i?.card?.info?.id} className='bg-slate-200 font-medium rounded-xl flex flex-col sm:flex-row  min-h-44 justify-between px-1 sm:px-5 py-2 border-b border-solid border-black'>
                    ...
                    <div className="img  w-40  sm:h-24 relative   ">
                        <img className='  object-cover h-36 none  w-40 cursor-pointer rounded-xl  ' src={"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_300,h_300,c_fit/" + i?.card?.info?.imageId} alt="" />
