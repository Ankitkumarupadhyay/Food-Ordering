...
const Categories = () => {
  ...
  <div className="main  2xl:w-[1200px] xl:w-[1100px] lg:w-[1000px] md:w-[760px] sm:w-[640px]    mx-auto  ">
    ...
    <div className="head py-2 sm:py-5  flex items-center justify-between">
       <div className='headCon sm:text-[35px] font-bold '> What's on your mind?</div>
        <div className=" flex">
            ...
            <div className='btn cursor-pointer sm:w-[40px] sm:h-[40px]   bg-[#e2e2e7] rounded-full mx-2 flex justify-center items-center ' onClick={()=> next()} >🔜</div>
    ...
    <div className="imgCon flex  overflow-hidden">
        {
            elements.map((elem)=>(
                <div style={{transform:`translateX(-${slide * 100}%)`}}  key={elem.id} className="cursor-pointer img 2xl:w-[170px] xl:w-[160px] lg:w-[145px] md:w-[110px] sm:w-[90px] duration-500 flex-grow shrink-0">
                    <img src={"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_288,h_360/" + elem?.imageId } alt="" />
                </div>
            ))
        }
    </div>
